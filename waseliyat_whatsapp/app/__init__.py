...
def create_app():
    ...
    app.config.update(
        ...
        TAKHFIID_ALLOWED_ORIGINS=os.getenv(
            'TAKHFIID_ALLOWED_ORIGINS',
            'https://zydan25.github.io,http://localhost:3000,http://localhost:5173'
        ),
    )
    ...
    def add_takhfid_cors_headers(response):
        if request.path.startswith('/takhfid/api/'):
            ...
            allowed = {
                item.strip().rstrip('/')
                for item in str(app.config['TAKHFIID_ALLOWED_ORIGINS']).split(',')
                if item.strip()
            }
