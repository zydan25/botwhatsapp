...
class WhatsAppSession {
  constructor({ name, rootDir, store, apiBaseUrl = null, backupPhone = null, description = '', webhookSecret = null, chromePath = null, io = null }) {
    this.name = name;
    ...
    this.chromePath = chromePath || process.env.CHROME_PATH || null;
    this.client = null;
    this.status = 'idle';
    this.qr = null;
    this.info = null;
  ...
  roomEmit(payload) {
    if (this.io) this.io.to(`session:${this.name}`).emit('session:update', payload);
  ...
  notification(text, level = 'info', extra = {}) {
    ...
    if (this.io) this.io.to(`session:${this.name}`).emit('session:notification', note);
  ...
  async buildClient() {
    ...
    return new Client({
      authStrategy: new LocalAuth({
        clientId: this.name,
        dataPath: path.join(this.rootDir, 'sessions')
      }),
      fetchMessages: false,
      puppeteer: {
        headless: true,
  ...
  async _startInternal() {
    ...
    try {
      ...
      this.lastEvent = { type: 'browser_error', at: nowIso() };
      await this.persistMeta({ lastEvent: this.lastEvent });
      await this.recordError('BROWSER_ERROR', error);
      throw error;
    }
    ...
    client.on('qr', async (qr) => {
      ...
      try {
        ...
        await this.recordError('QR_HANDLER_ERROR', error);
      }
    });
    ...
    client.on('message', async (message) => {
      ...
      try {
        ...
        await this.recordError('MESSAGE_HANDLER_ERROR', error);
      }
    });
    ...
    client.on('disconnected', async (reason) => {
      ...
      await this.persistMeta({ lastEvent: this.lastEvent, disconnectReason: this.lastError, qrAvailable: false });
  ...
  async _stopInternal() {
    ...
    if (client) {
      try {
        ...
        await this.recordError('STOP_ERROR', error);
      }
    }
    this.status = 'disconnected';
    this.qr = null;
  ...
  async regenerateQr() {
    if (this.status === 'connected') throw new Error('الجلسة متصلة. استخدم إعادة المصادقة لإنشاء QR جديد.');
    await this.stop();
    return this.start();
  }
  ...
  async reauthenticate() {
    await this.stop();
    await fs.remove(path.join(this.rootDir, 'sessions', `session-${this.name}`)).catch(() => {});
    this.status = 'idle';
    this.qr = null;
    this.info = null;
    this.lastError = null;
    this.lastEvent = { type: 'reauthenticate', at: nowIso() };
    await this.persistMeta({ status: 'idle', lastEvent: this.lastEvent });
    return this.start();
  }
  ...
  async sendToExternalApi(endpoint, payload) {
    ...
    try {
      const body = JSON.stringify({ ...payload, botId: this.name });
      const signature = `sha256=${crypto.createHmac('sha256', this.webhookSecret).update(body).digest('hex')}`;
      await axios.post(`${this.apiBaseUrl}${endpoint}`, body, {
        timeout: 15000,
        headers: {
          'content-type': 'application/json',
          'x-whatsapp-signature': signature,
          'x-whatsapp-event': endpoint.split('/').pop()
        },
  ...
  async getQrImageDataUrl() {
    return this.qr ? QRCode.toDataURL(this.qr, { errorCorrectionLevel: 'M', margin: 2, scale: 8 }) : null;
  }

  async getPublicStatus() {
    const [messages, errors, notifications] = await Promise.all([
      this.store.getMessages(this.name, 10, 0),
      this.store.getErrors(this.name, 10),
      this.store.getNotifications(this.name, 15)
    ]);
    return {
      ...this.serialize(),
      messages,
      ...
      qrDataUrl: this.qr ? await this.getQrImageDataUrl() : null
    };
  }
  ...
  getApiEndpoints() {
    ...
    return {
      ...
      reauthenticate: `${base}/reauthenticate`,
      logout: `${base}/logout`,
      delete: base,
      send: `${base}/send`,
...
class SessionManager {
  constructor({ rootDir, store, chromePath = null, io = null }) {
    this.rootDir = rootDir;
    this.store = store;
  ...
  async ensureSession(name, apiBaseUrl = null, backupPhone = null) {
    ...
    if (apiBaseUrl !== null && apiBaseUrl !== undefined && apiBaseUrl !== '') {
      session.apiBaseUrl = validateApiBaseUrl(apiBaseUrl);
      await session.persistMeta({ apiBaseUrl: session.apiBaseUrl });
    }
    if (backupPhone) {
  ...
  async createSession(name, options = {}) {
    if (await this.store.getSessionMeta(name) || this.sessions.has(name)) {
    ...
    await session.persistMeta({
      createdAt: nowIso(),
      clientUsername: options.clientUsername || null,
  ...
  async removeSession(name) {
    const session = this.sessions.get(name) || await this.ensureSession(name);
    try {
