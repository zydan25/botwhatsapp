...
class ClientStore {
  ...
  async getByUsername(username) {
    let safe;
    try { safe = this.sanitizeUsername(username); } catch { return null; }
  ...
  async save(record) {
    const safe = this.sanitizeUsername(record?.username);
    if (!record?.sessionName) throw new Error('الجلسة المرتبطة بالحساب غير موجودة');
