...
app.post('/api/client/profile', async (req, res) => {
  try {
    const { client } = await clientSession(req);
    const displayName = String(req.body?.displayName || '').trim().slice(0, 100);
    if (!displayName) throw new Error('اسم العرض مطلوب');
...
app.get('/api/client/integration.txt', async (req, res) => {
  try {
    const { client, s } = await clientSession(req);
    const sendUrl = `https://whatsapp.alattab.site/api/v1/sessions/${encodeURIComponent(s.name)}/send`;
    const statusUrl = `https://whatsapp.alattab.site/api/v1/sessions/${encodeURIComponent(s.name)}/status`;
    const receiveUrl = `${s.apiBaseUrl}/webhook/whatsapp`;
    const statusWebhook = `${s.apiBaseUrl}/webhook/session-status`;
    const qrWebhook = `${s.apiBaseUrl}/webhook/qr`;
    const key = getClientApiKey(s);
    const webhookSecret = s.webhookSecret;
    const content = [
      'WHATSAPP PRO — ملف ربط العميل',
      '================================',
      '',
      `اسم العميل: ${client.displayName || client.username}`,
      `اسم المستخدم: ${client.username}`,
      `اسم الجلسة: ${s.name}`,
      `حالة الجلسة: ${s.status}`,
      `رقم WhatsApp: ${s.info?.wid?.user || 'غير متصل بعد'}`,
      '',
      '1) API الإرسال',
      `رابط الإرسال: ${sendUrl}`,
      `API Key: ${key}`,
      'Header: x-api-key',
      '',
      'مثال cURL:',
      ...
      "  body: JSON.stringify({ phoneNumber: '9677xxxxxxx', message: 'مرحبا من نظامي' })",
      '});',
      ...
      `  headers={'x-api-key': '${key}', 'Content-Type': 'application/json'},`,
      "  json={'phoneNumber': '9677xxxxxxx', 'message': 'مرحبا من نظامي'})",
      'print(r.json())',
      '',
      'مثال PHP:',
      '$ch = curl_init();',
      `curl_setopt_array($ch, [CURLOPT_URL => '${sendUrl}', CURLOPT_POST => true,`,
      `  CURLOPT_HTTPHEADER => ['x-api-key: ${key}', 'Content-Type: application/json'],`,
      "  CURLOPT_POSTFIELDS => json_encode(['phoneNumber'=>'9677xxxxxxx','message'=>'مرحبا من نظامي']), CURLOPT_RETURNTRANSFER => true]);",
      ...
      '',
      '3) Webhook استقبال الرسائل',
      `URL: ${receiveUrl}`,
      'Method: POST',
      'Content-Type: application/json',
      `x-whatsapp-signature: sha256=<HMAC-SHA256 باستخدام Webhook Secret>`,
      `Webhook Secret: ${webhookSecret}`,
      '',
      'مثال الرسالة الواردة:',
      JSON.stringify({ session: s.name, botId: s.name, direction: 'in', messageId: 'message-id', from: '9677xxxxxxx@c.us', to: '9677xxxxxxx@c.us', body: 'مرحبا', type: 'chat', hasMedia: false, mediaSkipped: false, timestamp: new Date().toISOString() }, null, 2),
      '',
      '4) Webhook تغير الحالة',
      `URL: ${statusWebhook}`,
      'Method: POST',
      `مثال: { "session": "${s.name}", "status": "connected", "timestamp": "${new Date().toISOString()}" }`,
      '',
      '5) Webhook حدث QR',
      `URL: ${qrWebhook}`,
      'Method: POST',
      '',
      'ملاحظات الأمان:',
      '- API Key سرية ولا توضع في الواجهة الأمامية أو JavaScript العام.',
      '- Webhook Secret سرية وتستخدم للتحقق من توقيع Webhook.',
      '- رقم الهاتف يرسل بصيغة دولية بدون + أو مسافات، مثال اليمن: 9677xxxxxxx.',
      '- يجب أن تكون جلسة WhatsApp في حالة connected حتى ينجح الإرسال.',
      '',
      'Generated by WhatsApp Pro / whatsapp.alattab.site',
      `Session: ${s.name}`
    ].join('\n');
    res.setHeader('Content-Type', 'text/plain; charset=utf-8');
    res.setHeader('Content-Disposition', `attachment; filename="whatsapp-integration-${s.name}.txt"`);
    res.send(content);
  } catch (e) { jsonError(res, 400, e.message); }
});
